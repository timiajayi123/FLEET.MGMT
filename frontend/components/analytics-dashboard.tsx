...
function ChartDetail({ point, fallback, suffix }: { point: { label: string; value: number } | null; fallback: string; suffix: string }) {
  return <div className={`chart-hover-detail${point ? ' is-visible' : ''}`} aria-live="polite"><span>{point ? shortDate(point.label) : 'Chart details'}</span><strong>{point ? `${point.value}${suffix ? ` ${suffix}` : ''}` : fallback}</strong></div>;
}
