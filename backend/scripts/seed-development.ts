...
async function main() {
  ...
  const staffRole = await prisma.role.upsert({ where: { code: 'ST' }, update: { status: 'ACTIVE' }, create: { code: 'ST', name: 'Staff', isSystemRole: true, status: 'ACTIVE', sortOrder: 3 } });
  ...
  const staff = await ensureUser('staff@nmdpra.gov.ng', 'NMDPRA-STAFF-001', 'TEST STAFF', staffRole.id, process.env.DEV_STAFF_PASSWORD);
  ...
  const requestData = { staffName: staff.user.staffName, employeeId: staff.user.employeeId, locationId: location.id, directorateId: directorate.id, departmentId: department.id, unitId: unit.id, vehicleTypeId: vehicleType.id, requesterId: staff.user.id, location: location.name, directorate: directorate.name, department: department.name, unit: unit.name, purposeOfTrip: 'Development GPS workflow test', vehicleTypeName: vehicleType.name, destination: 'NMDPRA Abuja Field Office', departureDate: new Date(Date.now() + 30 * 60_000), expectedReturnDate: new Date(Date.now() + 4 * 60 * 60_000), numberOfPassengers: 2, priority: 'NORMAL' };
  const approvedRequest = await prisma.vehicleRequest.upsert({ where: { requestNumber: 'DEV-REQ-APPROVED' }, update: { ...requestData, status: 'APPROVED' }, create: { ...requestData, requestNumber: 'DEV-REQ-APPROVED', status: 'APPROVED' } });
  const activeRequest = await prisma.vehicleRequest.upsert({ where: { requestNumber: 'DEV-REQ-ACTIVE' }, update: { ...requestData, status: 'ALLOCATED' }, create: { ...requestData, requestNumber: 'DEV-REQ-ACTIVE', status: 'ALLOCATED' } });
  ...
  const upcomingRequest = await prisma.vehicleRequest.upsert({ where: { requestNumber: 'DEV-REQ-UPCOMING' }, update: { ...requestData, departureDate: new Date(Date.now() + 26 * 60 * 60_000), expectedReturnDate: new Date(Date.now() + 30 * 60 * 60_000), status: 'ALLOCATED' }, create: { ...requestData, requestNumber: 'DEV-REQ-UPCOMING', departureDate: new Date(Date.now() + 26 * 60 * 60_000), expectedReturnDate: new Date(Date.now() + 30 * 60 * 60_000), status: 'ALLOCATED' } });
  ...
  const completedRequest = await prisma.vehicleRequest.upsert({ where: { requestNumber: 'DEV-REQ-COMPLETED' }, update: { ...requestData, departureDate: new Date(Date.now() - 26 * 60 * 60_000), expectedReturnDate: new Date(Date.now() - 23 * 60 * 60_000), status: 'COMPLETED' }, create: { ...requestData, requestNumber: 'DEV-REQ-COMPLETED', departureDate: new Date(Date.now() - 26 * 60 * 60_000), expectedReturnDate: new Date(Date.now() - 23 * 60 * 60_000), status: 'COMPLETED' } });
  ...
  console.log(JSON.stringify({ created: { approvedRequest: approvedRequest.requestNumber, activeAllocation: active.id, upcomingAllocation: '10000000-0000-4000-8000-000000000003', completedTrip: completedTrip.id, vehicles: 3, drivers: 2 }, credentials: { admin: admin.password ? { email: admin.user.email, password: admin.password } : 'existing account retained', staff: staff.password ? { email: staff.user.email, password: staff.password } : 'existing account retained', driver: driverUser.password ? { email: driverUser.user.email, password: driverUser.password } : 'existing account retained' } }, null, 2));
}
