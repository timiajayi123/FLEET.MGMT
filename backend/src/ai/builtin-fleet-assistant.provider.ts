...
export class BuiltinFleetAssistantProvider {
  ...
  async ask(message: string, context: FleetAssistantContext): Promise<FleetAssistantResponse> {
    ...
    if (asksForCount(question, 'driver|chauffeur|operator|motorist')) return response('Driver count', `${dashboard.metrics.drivers} driver${dashboard.metrics.drivers === 1 ? ' is' : 's are'} registered in the fleet.`, [{ label: 'Registered drivers', value: String(dashboard.metrics.drivers) }, { label: 'Active drivers', value: String(dashboard.metrics.activeDrivers) }], { label: 'Open drivers', href: '/fleet/drivers' });
    if (asksForCount(question, 'vehicle|car|truck|bus|van|fleet')) return response('Vehicle count', `${dashboard.metrics.vehicles} vehicle${dashboard.metrics.vehicles === 1 ? ' is' : 's are'} registered in the fleet.`, [{ label: 'Registered vehicles', value: String(dashboard.metrics.vehicles) }, { label: 'Available vehicles', value: String(dashboard.metrics.availableVehicles) }], { label: 'Open vehicles', href: '/fleet/vehicles' });
    if (asksForCount(question, 'request|booking|application')) return response('Vehicle request count', `${dashboard.metrics.requests} vehicle request${dashboard.metrics.requests === 1 ? ' was' : 's were'} recorded in the selected period.`, [{ label: 'All requests', value: String(dashboard.metrics.requests) }, { label: 'Pending approval', value: String(dashboard.metrics.pendingRequests) }, { label: 'Approved or allocated', value: String(dashboard.metrics.approvedRequests) }], { label: 'Review requests', href: '/fleet/vehicle-requests/review' });
    if (asksForCount(question, 'trip|journey|travel|movement')) return response('Trip count', `${dashboard.metrics.trips} request-backed trip${dashboard.metrics.trips === 1 ? ' was' : 's were'} recorded in the selected period.`, [{ label: 'All trips', value: String(dashboard.metrics.trips) }, { label: 'Active trips', value: String(dashboard.metrics.activeTrips) }, { label: 'Completed trips', value: String(dashboard.metrics.completedTrips) }], { label: 'Open trips', href: '/fleet/trips' });
    ...
    if (/available|availability|unavailable|free|idle|maintenance|service|repair|workshop|in use|busy|deployed/.test(question)) return response('Vehicle availability', `${dashboard.metrics.availableVehicles} vehicles are available, ${dashboard.metrics.inUseVehicles} are in use, and ${dashboard.metrics.maintenanceVehicles} are under maintenance.`, availability(dashboard.metrics));
...
function asksForCount(question: string, subject: string) { return new RegExp(`(?:how many|number of|count|total|registered|all)\\s+(?:[a-z ]*\\s+)?(?:${subject})`, 'i').test(question); }
