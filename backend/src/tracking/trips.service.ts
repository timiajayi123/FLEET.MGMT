...
export class TripsService {
  ...
  list(user: { employeeId: string; role: { code: string } }, from?: Date, to?: Date) {
    const where =
      user.role.code === 'DRIVER'
        ? { driver: { employeeId: user.employeeId }, requestId: { not: null } }
        : { requestId: { not: null } };
    const dateWhere = from || to ? { startedAt: { ...(from ? { gte: from } : {}), ...(to ? { lte: to } : {}) } } : {};
  ...
  async start(allocationId: string, employeeId: string, coordinate: TripCoordinateDto) {
    return this.prisma.$transaction(async (tx) => {
      const allocation = await tx.vehicleAllocation.findUnique({
        where: { id: allocationId },
        include: { driver: true, trip: true, request: true },
      });
      if (!allocation || allocation.driver.employeeId !== employeeId)
        throw new NotFoundException('Assignment not found for this driver.');
      if (!['ASSIGNED', 'ACCEPTED'].includes(allocation.status))
        throw new BadRequestException('This assignment cannot be started.');
      if (!allocation.requestId || allocation.request?.status !== 'ALLOCATED') {
        throw new BadRequestException(
          'Live tracking can only start after an approved vehicle request has been allocated to this driver.',
        );
      }
      const now = new Date();
      const trip = allocation.trip
        ? await tx.trip.update({
            where: { id: allocation.trip.id },
            data: {
              status: 'IN_PROGRESS',
              startedAt: now,
  ...
  async end(tripId: string, employeeId: string, coordinate: TripCoordinateDto) {
    return this.prisma.$transaction(async (tx) => {
      const trip = await tx.trip.findUnique({
        where: { id: tripId },
        include: { driver: true, allocation: true },
      });
      if (!trip || trip.driver.employeeId !== employeeId)
      ...
      const points = await tx.driverLocationHistory.findMany({
        where: { tripId },
      ...
      const completed = await tx.trip.update({
        ...
        data: {
          status: 'COMPLETED',
          endedAt,
          endLatitude: coordinate.latitude ?? points.at(-1)?.latitude,
          endLongitude: coordinate.longitude ?? points.at(-1)?.longitude,
      ...
      await Promise.all([
        tx.vehicleAllocation.update({
          where: { id: trip.allocationId },
          data: { status: 'COMPLETED', actualEndAt: endedAt },
        }),
  ...
  async history(tripId: string, user: { employeeId: string; role: { code: string } }) {
    const trip = await this.prisma.trip.findUnique({
      ...
      include: {
        driver: true,
        allocation: { include: { vehicle: true, request: true } },
        locationHistory: { orderBy: { recordedAt: 'asc' } },
      },
    });
    if (!trip) throw new NotFoundException('Trip not found.');
    if (user.role.code === 'DRIVER' && trip.driver.employeeId !== user.employeeId)
      throw new NotFoundException('Trip not found.');
    if (!['DRIVER', 'S_ADMIN', 'FM'].includes(user.role.code))
      throw new NotFoundException('Trip not found.');
    return trip;
  }
}
