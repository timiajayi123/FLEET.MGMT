...
export class TrackingController {
  ...
  @Get('trips') async listTrips(@Req() req: Request, @Query() query: Record<string, string>) { const from = query.from ? new Date(query.from) : undefined; const to = query.to ? new Date(query.to) : undefined; if (to) to.setUTCHours(23, 59, 59, 999); return { data: await this.trips.list(await requireUser(this.auth, req, ['S_ADMIN', 'FM', 'DRIVER']), from && !Number.isNaN(from.getTime()) ? from : undefined, to && !Number.isNaN(to.getTime()) ? to : undefined) }; }
