...
export class FuelService {
  ...
  async bootstrap(user: SessionUser) {
    ...
    const activeAllocation = driver ? await this.prisma.vehicleAllocation.findFirst({ where: { driverId: driver.id, status: { in: ['ASSIGNED', 'ACCEPTED', 'IN_PROGRESS'] } }, orderBy: { startAt: 'desc' }, include: { vehicle: { select: { id: true, registrationNumber: true, manufacturer: true, model: true, vehicleType: { select: { name: true } } } }, driver: { select: { id: true, staffName: true, employeeId: true } }, trip: { select: { id: true, status: true, calculatedDistance: true } } } }) : null;
