...
export class FuelController {
  ...
  @Get('bootstrap') async bootstrap(@Req() req: Request) {
    return this.fuel.bootstrap(await requireUser(this.auth, req, ['S_ADMIN', 'FM', 'DRIVER']));
  }
  @Get('entries') async entries(@Req() req: Request) {
    return this.fuel.list(await requireUser(this.auth, req, ['S_ADMIN', 'FM', 'DRIVER']));
  }
  @Get('dashboard') async dashboard(@Req() req: Request, @Query() query: Record<string, string>) {
    await requireUser(this.auth, req, [...FLEET_MANAGER_ROLES]);
    return this.fuel.dashboard(query);
  }
  @Get('comparison') async comparison(@Req() req: Request, @Query() query: Record<string, string>) {
    await requireUser(this.auth, req, [...FLEET_MANAGER_ROLES]);
    return this.fuel.comparison(query);
  }
  @Post('entries')
  @UseInterceptors(
  ...
  async create(
    ...
    return this.fuel.create(
      dto,
      await requireUser(this.auth, req, ['S_ADMIN', 'FM', 'DRIVER']),
      files ?? {},
    );
  }
  @Patch('entries/:id/decision') async decision(
    @Req() req: Request,
    @Param('id') id: string,
    @Body() dto: DecisionDto,
  ) {
    return this.fuel.decide(id, dto, await requireUser(this.auth, req, [...FLEET_MANAGER_ROLES]));
  }
  @Get('cards') async cards(@Req() req: Request) {
    await requireUser(this.auth, req, [...FLEET_MANAGER_ROLES]);
    return this.fuel.cards();
  }
  ...
  @Patch('baselines/:vehicleId') async baseline(
    @Req() req: Request,
    @Param('vehicleId') vehicleId: string,
    @Body() dto: BaselineDto,
  ) {
    return this.fuel.saveBaseline(
      vehicleId,
      dto,
      await requireUser(this.auth, req, [...FLEET_MANAGER_ROLES]),
    );
