...
export class MaintenanceService {
  ...
  async vehicles(user: SessionUser) {
    ...
    if (!driver)
      throw new NotFoundException('Your user account is not linked to a driver profile.');
  ...
  async create(
    ...
    if (Number.isNaN(occurredAt.getTime()) || occurredAt > new Date())
      throw new BadRequestException('Issue date must be a valid past or current date.');
    if (evidence && !['image/jpeg', 'image/png', 'image/webp'].includes(evidence.mimetype))
      throw new BadRequestException('Evidence image must be JPEG, PNG, or WebP.');
    await this.assertVehicleAllowed(dto.vehicleId, user);
    return {
      data: await this.prisma.maintenanceRequest.create({
        data: {
          vehicleId: dto.vehicleId,
          reportedById: user.id,
          issueType: dto.issueType.trim(),
          issueDescription: dto.issueDescription.trim(),
          issueOccurredAt: occurredAt,
          ...(evidence
            ? {
                evidenceMimeType: evidence.mimetype,
                evidenceData: Uint8Array.from(evidence.buffer),
              }
            : {}),
        },
        include,
  ...
  async evidence(id: string) {
    const request = await this.prisma.maintenanceRequest.findUnique({
      where: { id },
      select: { evidenceMimeType: true, evidenceData: true },
    });
    if (!request?.evidenceData || !request.evidenceMimeType)
      throw new NotFoundException('No maintenance evidence image was attached to this request.');
    return request;
  }
  ...
  private async assertVehicleAllowed(vehicleId: string, user: SessionUser) {
    if (user.role.code !== 'DRIVER') {
      if (
        !(await this.prisma.vehicle.findUnique({ where: { id: vehicleId }, select: { id: true } }))
      )
    ...
    if (!allocated)
      throw new BadRequestException(
        'Drivers can report only vehicles previously allocated to them.',
      );
  }
}
